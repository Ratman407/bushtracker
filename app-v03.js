const STORAGE_KEY = 'bushtrack-v03';
const OLD_KEYS = ['bushtrack-v01', 'bushtrack-v02'];
...
function newConditions(){
  return {
    ...
    weather: weighted([{v:'Clear',w:26},{v:'Overcast',w:24},{v:'Showers',w:18},{v:'Windy',w:18},{v:'Hot',w:14}]),
    wind: weighted([{v:'Light',w:35},{v:'Steady',w:40},{v:'Gusty',w:25}])
  };
}
...
function initialState(){
  ...
  return {
    version:3,xp:0,cash:0,totalSteps:0,claimCount:0,lastClaimDate:null,stepCredit:0,
    currentRegion:'Home block',conditions:newConditions(),
    active:{kind:'hunt',id:null},
    hunt:{id:uid('hunt'),signFamily:'deer',title:'Fresh deer sign',startDistance:firstDist,remaining:firstDist,cumulative:0,called:false,stage:'track'},
    ...
    events:[{title:'Fresh sign',text:'Fresh deer tracks cut across the track and disappear into broken timber.',date:displayDay()}]
  };
}
...
function migrateOld(old){
  ...
  s.xp=old.xp||0; s.cash=old.cash||0; s.lastClaimDate=old.lastClaimDate||null;
  s.events=Array.isArray(old.events)?old.events.map(e=>({...e,date:e.date==='Today'?displayDay():e.date})):s.events;
  s.trophies=Array.isArray(old.trophies)?old.trophies.map(t=>({id:uid('legacy'),type:(t.name||'').toLowerCase().includes('bass')?'fish':'animal',species:t.name||'Legacy trophy',name:t.name||'Legacy trophy',detail:t.detail||'',metricValue:0,metricLabel:'',date:displayDay()})):[];
  if(old.hunt){
    s.hunt.startDistance=old.hunt.startDistance||10000;
    s.hunt.remaining=old.hunt.remaining ?? s.hunt.startDistance;
    s.hunt.cumulative=old.hunt.cumulative||0;
    if(old.hunt.ready){
      s.hunt.remaining=0; s.hunt.stage='encounter';
      s.currentEncounter={id:uid('enc'),source:'main',resumeActive:null,group:[{id:uid('animal'),species:'Fallow deer',sex:'Buck',quality:'good',metricValue:old.hunt.trophyPotential||220,metricLabel:`~${old.hunt.trophyPotential||220} pt`,nickname:null}],stage:'choose',selectedAnimal:null,bonusSteps:0,approachRemaining:0,approachStart:0,shotRange:null,recoveryRemaining:0,recoveryStart:0};
    }
  }
  if(old.river){
    s.river.discovered=!!old.river.discovered; s.river.startDistance=old.river.startDistance||8500; s.river.remaining=old.river.remaining ?? 8500; s.river.ready=!!old.river.ready;
    if(s.river.discovered) ensureLocation(s,'creek');
    if(s.river.ready){ const loc=ensureLocation(s,'deepHole'); loc.fishReady=true; }
  }
  if(old.active==='river' && s.river.discovered && !s.river.ready) s.active={kind:'river',id:null};
  addEventTo(s,'V0.3 upgrade','Your existing BushTrack progress was carried into the new world system.');
  return s;
}
...
function loadState(){
  try{
    ...
    if(raw) return deepMerge(initialState(),JSON.parse(raw));
    for(const key of OLD_KEYS){
      const oldRaw=localStorage.getItem(key);
      if(oldRaw){ const migrated=migrateOld(JSON.parse(oldRaw)); localStorage.setItem(STORAGE_KEY,JSON.stringify(migrated)); return migrated; }
    }
...
function addEventTo(s,title,text){ s.events.unshift({title,text,date:displayDay()}); s.events=s.events.slice(0,70); }
function addEvent(title,text){ addEventTo(state,title,text); }
...
function ensureLocation(s,templateId){
  ...
  loc={id:uid('loc'),templateId,type:t.type,name:t.name,desc:t.desc,actions:[...t.actions],fishery:t.fishery||null,fishReady:templateId==='deepHole',returnDistance:randInt(1400,3200),camp:false,pressure:0,discoveredClaim:s.claimCount};
...
function refreshConditions(){ state.conditions=newConditions(); }
...
function render(){
  ...
  $('#claimStatus').textContent=claimed?'Today’s steps have already been claimed.':'No steps claimed today.';
  $('#claimBtn').disabled=claimed; $('#claimBtn').style.opacity=claimed?'.45':'1';
  ...
  renderHunt(); renderRiver(); renderObjectives(); renderPendingChoice(); renderEncounterStatus(); renderLocations(); renderKnownAnimals(); renderFieldGear(); renderInventory(); renderShop(); renderKnowledge(); renderJournal();
  save();
}
...
function renderJournal(){
  $('#eventLog').innerHTML=state.events.map(e=>`<div class="log-entry"><strong>${esc(e.title)}</strong><div>${esc(e.text)}</div><small>${esc(e.date)}</small></div>`).join('');
...
function claimSteps(){
  const today=localDayKey(); if(state.lastClaimDate===today)return;
  const steps=parseInt(String($('#stepsInput').value).replace(/\D/g,''),10);
  if(!steps||steps<1||steps>100000){ $('#claimStatus').textContent='Put in a sensible step count first.'; return; }
  state.lastClaimDate=today; state.claimCount++; state.totalSteps+=steps; state.xp+=Math.round(steps/22); refreshConditions();
  matureFieldGear(); expireOldObjectives();
  applyStepsToActive(steps);
  ambientDiscovery(steps); maybeBranchEncounter(steps); maybeGatherBait(steps); maybeFindTackle(steps);
  $('#stepsInput').value=''; render();
}
...
function advanceHunt(steps){
  ...
  state.hunt.cumulative+=steps; state.hunt.remaining-=steps;
  ...
  if(state.hunt.remaining<=0){
    ...
    const relocation=Math.max(0,Math.min(state.hunt.remaining-300,randInt(180,780))); if(relocation>0) state.hunt.remaining-=relocation;
    addEvent('Tracks getting fresher',`${fmt(steps)} steps on the trail. The moving animals shifted the intercept another ${fmt(relocation)} steps in your favour. About ${fmt(state.hunt.remaining)} remain.`);
  }
}
...
function newMainHunt(opts={}){
  ...
  state.hunt={id:uid('hunt'),signFamily:family,title:knownId?'Distinctive familiar sign':titles[family],startDistance:distance,remaining:distance,cumulative:0,called:false,stage:'track',knownAnimalId:knownId};
...
function createAnimalEncounter(species,opts={}){
  ...
  if(opts.knownAnimalId){
    const ka=state.knownAnimals.find(x=>x.id===opts.knownAnimalId); if(ka) group=[{...ka,id:uid('animal'),knownId:ka.id}];
  }
...
function generateAnimal(species,forceMature=false){
  let a={id:uid('animal'),species,sex:'',quality:'average',metricValue:0,metricLabel:'',nickname:null};
  if(species==='Fallow deer'){
    const type=forceMature?'mature buck':weighted([{v:'doe',w:42},{v:'yearling',w:14},{v:'spiker',w:12},{v:'buck',w:24},{v:'mature buck',w:7},{v:'trophy buck',w:1}]);
    a.sex=type.includes('doe')?'Doe':type.includes('yearling')?'Yearling':type.includes('spiker')?'Spiker':'Buck';
    a.quality=type.includes('trophy')?'trophy':type.includes('mature')?'mature':type==='buck'?'good':type;
    if(a.sex==='Buck'){ a.metricValue=a.quality==='trophy'?randInt(255,310):a.quality==='mature'?randInt(220,270):a.quality==='good'?randInt(175,235):randInt(80,150); a.metricLabel=`~${a.metricValue} pt`; }
  }else if(species==='Red deer'){
    const autumnBoost=state.conditions.season==='Autumn'?1:0;
    const type=weighted([{v:'hind',w:38-autumnBoost*5},{v:'young stag',w:22},{v:'stag',w:30+autumnBoost*2},{v:'mature stag',w:8+autumnBoost*2},{v:'trophy stag',w:2+autumnBoost}]);
    a.sex=type==='hind'?'Hind':'Stag'; a.quality=type==='trophy stag'?'trophy':type==='mature stag'?'mature':type==='stag'?'good':type; if(a.sex==='Stag'){a.metricValue=a.quality==='trophy'?randInt(300,380):a.quality==='mature'?randInt(260,325):a.quality==='good'?randInt(210,285):randInt(120,210);a.metricLabel=`~${a.metricValue} pt`;}
  }else if(species==='Feral goat'){
    const type=weighted([{v:'nanny',w:34},{v:'young billy',w:24},{v:'billy',w:32},{v:'wide billy',w:10}]); a.sex=type==='nanny'?'Nanny':'Billy'; a.quality=type==='wide billy'?'trophy':type==='billy'?'good':type; if(a.sex==='Billy'){a.metricValue=a.quality==='trophy'?randInt(90,118):a.quality==='good'?randInt(72,98):randInt(45,78);a.metricLabel=`~${a.metricValue} cm horn spread`;}
  }else if(species==='Feral pig'){
    const type=weighted([{v:'sow',w:34},{v:'young boar',w:22},{v:'boar',w:35},{v:'old boar',w:9}]); a.sex=type==='sow'?'Sow':'Boar'; a.quality=type==='old boar'?'trophy':type==='boar'?'good':type; a.metricValue=a.sex==='Boar'?(a.quality==='trophy'?randInt(110,165):a.quality==='good'?randInt(75,125):randInt(45,85)):randInt(45,110); a.metricLabel=`~${a.metricValue} kg`;
  }else{ a.sex='Rabbit'; a.quality=chance(.08)?'trophy':'average'; a.metricValue=randInt(1,3); a.metricLabel=`~${a.metricValue}.${randInt(0,9)} kg`; }
  if(a.species==='Fallow deer'&&chance(.004)){ a.variant=chance(.5)?'Albino':'Melanistic'; a.quality='trophy'; a.nickname=`${a.variant} fallow`; registerKnownAnimal(a); }
  if((a.quality==='mature'||a.quality==='trophy')&&chance(.4)&&!a.nickname){ a.nickname=makeNickname(a); registerKnownAnimal(a); }
  return a;
}
function makeNickname(a){
  if(a.species==='Fallow deer') return pick(['Split-tine buck','Wide buck','Dark-antler buck','Old palmated buck']);
  if(a.species==='Red deer') return pick(['Heavy stag','Broken-tine stag','Dark-maned stag','Old back-block stag']);
  if(a.species==='Feral pig') return pick(['Scarred boar','Black boar','Old creek boar','Heavy boar']);
  if(a.species==='Feral goat') return pick(['Wide billy','Broken-tip billy','White billy','Old ridge billy']);
  return 'Big rabbit';
}
function registerKnownAnimal(a){
  if(!a.nickname||state.knownAnimals.some(x=>x.nickname===a.nickname&&x.species===a.species)) return;
  state.knownAnimals.push({id:uid('known'),species:a.species,sex:a.sex,quality:a.quality,metricValue:a.metricValue,metricLabel:a.metricLabel,nickname:a.nickname,status:'At large',lastSeen:displayDay()});
}
...
function openApproach(){
  ...
  $('#approachModal').dataset.routes=JSON.stringify(routes); $('#approachModal').classList.remove('hidden');
}
function approachOptions(){
  const boots=state.gear.boots||0; const windPenalty=state.conditions.wind==='Gusty'?.04:state.conditions.wind==='Steady'?.01:0;
  return [
    {name:'Straight in',steps:randInt(550,850),range:[175,245],spook:clamp(.27+windPenalty-boots*.035,.08,.4),desc:'Shortest walk, but you are relying on cover and luck.'},
    {name:'Use the timber',steps:randInt(1100,1650),range:[105,175],spook:clamp(.14+windPenalty-boots*.03,.04,.28),desc:'Longer line through cover with a better wind.'},
    {name:'Circle right around',steps:randInt(1900,2700),range:[60,115],spook:clamp(.06+windPenalty-boots*.018,.015,.16),desc:'A proper downwind circle. More walking for the best chance and closest shot.'}
  ];
}
function chooseApproach(i){
  const e=state.currentEncounter; if(!e)return; const routes=JSON.parse($('#approachModal').dataset.routes||'[]'); const r=routes[i]; if(!r)return;
...
function rememberEscape(a,reason){
  ...
  if(a.knownId) known=state.knownAnimals.find(x=>x.id===a.knownId);
  ...
  if(known){ known.status='At large'; known.lastSeen=displayDay(); addEvent('One to remember',`${known.nickname} is still out there.`); }
}
...
function harvestAnimal(a,how){
  ...
  if(a.knownId){ const k=state.knownAnimals.find(x=>x.id===a.knownId); if(k)k.status='taken'; }
...
function maybeBranchEncounter(steps){
  ...
  state.pendingEncounter={id:uid('pending'),kind:'animal',species,title:`Fresh ${species.toLowerCase()} sign ${context}`,text:`You were heading somewhere else, but this sign is fresh. Following it will pause your current plan rather than delete it.`,distance:randInt(1200,4200),chaseLabel:`Chase ${species.toLowerCase()}`};
}
...
function createObjective({type,title,story,distance,tag,data={}}){
  const o={id:uid('obj'),type,title,story,startDistance:distance,remaining:distance,tag,data,complete:false,createdClaim:state.claimCount}; state.objectives.push(o); return o;
}
...
function expireOldObjectives(){
  for(const o of state.objectives){ if(o.expiresAtClaim&&state.claimCount>o.expiresAtClaim&&!o.complete){o.complete=true;addEvent('Sign went cold',`${o.title} faded before you got back to it.`);} }
...
function ambientDiscovery(steps){
  if(steps<1800)return; const discoveryChance=clamp(.11+steps/26000+(state.gear.pack||0)*.025,.12,.46); if(!chance(discoveryChance))return;
...
function placeCamera(loc){
  ...
  state.inventory.trailCameras--; state.cameras.push({id:uid('cam'),locationId:loc.id,placedClaim:state.claimCount,ready:false,lastPhoto:null}); addEvent('Camera set',`Trail camera left at ${loc.name}. It needs at least another day before it is worth walking back to check.`);render();
}
function matureFieldGear(){
  for(const c of state.cameras) if(state.claimCount>c.placedClaim)c.ready=true;
  for(const n of state.nets) if(state.claimCount>n.placedClaim)n.ready=true;
  for(const loc of state.locations){ if(loc.fishery){ loc.pressure=Math.max(0,(loc.pressure||0)-(state.conditions.weather==='Showers'?.12:.05)); } }
}
function startCameraCheck(loc){
  ...
  const o=createObjective({type:'cameraCheck',title:`Check camera at ${loc.name}`,story:'The camera may have something, but you still need to walk back in to pull the card.',distance:loc.returnDistance,tag:'CAMERA',data:{cameraId:cam.id,resumeActive:{...state.active}}});setObjectiveActive(o.id,true);render();
}
function checkCameraNow(camId){
  const cam=state.cameras.find(c=>c.id===camId);if(!cam)return;const loc=getLoc(cam.locationId);cam.ready=false;cam.placedClaim=state.claimCount;
...
function placeNet(loc){
  ...
  state.inventory.yabbyNets--;state.nets.push({id:uid('net'),locationId:loc.id,placedClaim:state.claimCount,ready:false});addEvent('Yabby net set',`A net is baited and left at ${loc.name}. Walk back after another day to see what found it.`);render();
}
function startNetCheck(loc){
  ...
  const o=createObjective({type:'netCheck',title:`Check yabby net at ${loc.name}`,story:'The net has soaked long enough. Time to physically walk back and check it.',distance:loc.returnDistance,tag:'NET',data:{netId:net.id,resumeActive:{...state.active}}});setObjectiveActive(o.id,true);render();
}
function checkNetNow(netId){
  const net=state.nets.find(n=>n.id===netId);if(!net)return;const loc=getLoc(net.locationId);net.ready=false;net.placedClaim=state.claimCount;const n=weighted([{v:0,w:12},{v:1,w:18},{v:2,w:25},{v:3,w:22},{v:4,w:14},{v:5,w:9}]);
...
function openFishing(locId){
  ...
  if(!state.fishSession||state.fishSession.locationId!==locId||state.fishSession.ended){
    const snagAt=randInt(11,21);state.fishSession={locationId:locId,snagAt,casts:0,caught:0,ended:false,log:[]};
  }
...
function landFish(fish){
  ...
  state.pendingFish=null;renderFishingModal();render();
}
function endFishingOnSnag(){
  const s=state.fishSession;s.ended=true;const loc=getLoc(s.locationId);let lost='';if(selectedLure!=='Bait'&&chance(.7)&&(state.inventory[selectedLure]||0)>0){state.inventory[selectedLure]--;lost=` and lost the ${selectedLure.toLowerCase()}`;}s.log.push(`Cast ${s.casts}: Buried in the snag${lost}. Session over.`);$('#fishResult').textContent=`Buried it in the timber${lost}. That ends this visit.`;loc.fishReady=false;loc.returnDistance=randInt(1400,3200);loc.pressure=clamp((loc.pressure||0)+.14,0,.6);addEvent('Snagged up',`After ${s.casts-1} usable casts and ${s.caught} fish, the lure finally found the timber. You will have to walk back another day for another session.`);renderFishingModal();save();
}
...
$('#resetBtn').addEventListener('click',()=>{if(confirm('Reset ALL BushTrack progress?')){OLD_KEYS.forEach(k=>localStorage.removeItem(k));localStorage.removeItem(STORAGE_KEY);state=initialState();save();render()}});
...
render();
if(imported>0&&imported<100000){history.replaceState({},'',location.pathname);if(autoClaim){if(state.lastClaimDate===localDayKey())$('#claimStatus').textContent=`${fmt(imported)} steps received from Shortcut, but today is already claimed.`;else{$('#stepsInput').value=imported;claimSteps();}}else{$('#stepsInput').value=imported;$('#claimStatus').textContent=`${fmt(imported)} steps imported — tap Claim steps when ready.`;}}
