...
const STORAGE_KEY = 'bushtrack-v04';
const BACKUP_KEY = 'bushtrack-backups-v04';
const LEGACY_KEYS = ['bushtrack-v03','bushtrack-v02','bushtrack-v01'];
const MAX_BACKUPS = 5;
...
function initialState(){
  ...
  return {
    version:4,schemaVersion:SCHEMA_VERSION,
    meta:{saveId,currentDay:day,createdAt:now,lastSavedAt:null,lastBackupAt:null,lastBackupReason:null,randomCounter:0,lastVersion:APP_VERSION,storageHealthy:true,lastDayProcessedAt:now},
...
function upgradeLegacy(old,sourceKey){
  ...
  addEventTo(s,'V0.4 upgrade',`Save upgraded from ${sourceKey.replace('bushtrack-','').toUpperCase()}. Incremental step claims and backup protection are now active.`,'system');
  return s;
}
...
function recoverFromBackups(){
  ...
  return deepMerge(initialState(),good[0].data);
}
function loadState(){
  try{
    ...
    if(raw){
      try{
        ...
        if(validateState(parsed)){
          ...
          upgraded.schemaVersion=SCHEMA_VERSION;upgraded.version=4;upgraded.meta.lastVersion=APP_VERSION;
          return upgraded;
        }
...
state=loadState();
processDayRollover();
if(!readBackups().length)createBackup('Initial V0.4 safety backup');
...
function safeSetItem(key,value){
  try{localStorage.setItem(key,value);runtimeStorageError=null;if(state?.meta)state.meta.storageHealthy=true;return true;}
...
function save(){
  ...
  state.version=4;state.schemaVersion=SCHEMA_VERSION;
  state.meta=state.meta||{};
  state.meta.lastSavedAt=new Date().toISOString();state.meta.lastVersion=APP_VERSION;
  const ok=safeSetItem(STORAGE_KEY,JSON.stringify(state));
  if(ok)lastRuntimeSaveAt=state.meta.lastSavedAt;
...
function createBackup(reason='Automatic backup'){
  ...
  try{
    ...
    const envelope={kind:'BushTrackBackup',schemaVersion:SCHEMA_VERSION,appVersion:APP_VERSION,createdAt:new Date().toISOString(),reason,saveId:state.meta?.saveId||'',data:snapshot};
...
function processDayRollover(){
  ...
  for(let i=1;i<=capped;i++){
    ...
    matureFieldGear(day);expireOldObjectives(day);
    if(state.pendingEncounter?.createdDate&&state.pendingEncounter.createdDate<day){
      addEvent('Opportunity moved on',`${state.pendingEncounter.species||'The animal'} sign you had noticed yesterday has gone cold.`,'world');
  ...
  if(days>capped){
    ...
    matureFieldGear(today);expireOldObjectives(today);
    state.meta.currentDay=today;state.conditions=conditionsForSeed(state.meta.saveId,today);
...
function testStorage(){
  try{
    const k='__bushtrack_probe__';localStorage.setItem(k,'1');localStorage.removeItem(k);
...
function saveExportEnvelope(){
  ...
  const envelope={kind:'BushTrackSave',schemaVersion:SCHEMA_VERSION,appVersion:APP_VERSION,exportedAt:new Date().toISOString(),saveId:state.meta.saveId,data};
...
async function exportDiagnostics(){
  ...
  const diag={
    ...
    totalSteps:state.totalSteps,xp:state.xp,level:level(),eventCount:state.events?.length||0,locationCount:state.locations?.length||0,
...
function confirmImport(){
  ...
  state=incoming;state.schemaVersion=SCHEMA_VERSION;state.version=4;state.meta.lastVersion=APP_VERSION;
...
function undoLastStepClaim(){
  ...
  state.schemaVersion=SCHEMA_VERSION;state.version=4;state.meta.lastVersion=APP_VERSION;
...
function fullWipe(){
  ...
  LEGACY_KEYS.forEach(k=>localStorage.removeItem(k));localStorage.removeItem(STORAGE_KEY);
...
function renderSettings(){
  ...
  $('#historyCountText').textContent=`${fmt(state.events?.length||0)} permanent journal events stored`;
...
function ensureLocation(s,templateId){
  ...
  loc={id:uid('loc'),templateId,type:t.type,name:t.name,desc:t.desc,actions:[...t.actions],fishery:t.fishery||null,fishReady:templateId==='deepHole',returnDistance:randInt(1400,3200),camp:false,pressure:0,discoveredClaim:s.claimCount,discoveredDate:s.meta?.currentDay||localDayKey()};
...
function getTodayLedger(){
  ...
  if(!state.stepLedger[day])state.stepLedger[day]={date:day,healthTotal:0,credited:0,claims:[],firstClaimAt:null,lastClaimAt:null};
...
function creditHealthTotal(currentTotal,{source='manual',skipBackup=false}={}){
  ...
  ledger.claims=(ledger.claims||[]);ledger.claims.push({at:new Date().toISOString(),source,total:currentTotal,delta});
  state.lastClaimDate=today;state.totalSteps=Number(state.totalSteps||0)+delta;state.xp+=Math.round(delta/22);
...
function createObjective({type,title,story,distance,tag,data={},expiresDays=null}){
  ...
  if(expiresDays==null&&type==='branchHunt')expiresDays=2;
  ...
  state.objectives.push(o);return o;
}
...
function placeCamera(loc){
  ...
  state.inventory.trailCameras--; state.cameras.push({id:uid('cam'),locationId:loc.id,placedClaim:state.claimCount,placedDate:localDayKey(),ready:false,lastPhoto:null}); addEvent('Camera set',`Trail camera left at ${loc.name}. It needs at least another day before it is worth walking back to check.`);render();
}
...
function placeNet(loc){
  ...
  state.inventory.yabbyNets--;state.nets.push({id:uid('net'),locationId:loc.id,placedClaim:state.claimCount,placedDate:localDayKey(),ready:false});addEvent('Yabby net set',`A net is baited and left at ${loc.name}. Walk back after another day to see what found it.`);render();
}
